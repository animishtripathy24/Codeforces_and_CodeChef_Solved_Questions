// Problem: Akash and Dinner
// Contest: CodeChef - START69
...
int32_t main(){
	...
	while(T--)
	{
		...
		else
		{
			...
			for(int i=0;i<n;i++)
			{
				if(mp.find(category[i])==mp.end())
				{
					mp.insert(make_pair(category[i],time[i]));
				}
				else
				{
					mp[category[i]]=min(mp[category[i]],time[i]);
				}
			}
			// for(auto &value:mp)
			// {
